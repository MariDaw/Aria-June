<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <form action="{{ route('productos.update', $producto->id) }}" method="PUT" enctype="multipart/form-data">
        @method('PUT')
        @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                <div class="grid grid-cols-1">
                    ...
                    <div class="grid grid-cols-1">
                        <label for="precio"
                        class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold @error('precio') text-red-500 @enderror">
                        Precio:
                    </label>
                    <input type="text" name="precio" required
                        class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent @error('precio') border-red-500 @enderror"
                        value="{{ old('precio', $producto->precio) }}">
                    </div>
            </div>

            <div class="grid grid-cols-1 mt-5 mx-7">
                <img src="/img/Tienda{{ $producto->imagen }}" width="200px" id="imagenSeleccionada">
            </div>

            <div class="grid grid-cols-1 mt-5 mx-7">
            <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold mb-1">Subir Imagen</label>
                <div class='flex items-center justify-center w-full'>
                    <label class='flex flex-col border-4 border-dashed w-full h-32 hover:bg-gray-100 hover:border-purple-300 group @error('imagen') text-red-500 @enderror'>
                    ...
                    <input name="imagen" id="imagen" type='file' class="hidden" @enderror"  value="{{ old('imagen', $producto->imagen) }}"/>
