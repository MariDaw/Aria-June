                ...
                <form action="{{ route('link.store', $publicacion->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                            ...
                            <div class="grid grid-cols-1">
                                <label for="publicacion_id"
                                    class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold @error('publicacion_id') text-red-500 @enderror">
                                    publicacion_id:
                                </label>
                                <input type="text" name="publicacion_id"  required
                                    class="py-2 px-3 rounded-lg border-2 border-purple-300 mt-1 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent @error('publicacion_id') border-red-500 @enderror"
                                    value="{{ old('publicacion_id', $link->publicacion_id) }}">
                            </div>
